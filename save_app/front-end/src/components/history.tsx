    ...
    <div className="flex items-center justify-center min-h-screen bg-background text-foreground p-6">
      <div className="relative w-full max-w-2xl bg-card/60 backdrop-blur rounded-xl shadow-lg p-8">
       ...
       home
        ...
        <h1 className="text-2xl font-bold mb-4">Historique des sauvegardes</h1>
        {history.length === 0 ? (
          ...
          <div key={index} className="border-b border-border py-4">
            <h2 className="text-lg font-semibold">{backup.filename}</h2>
            <p className="text-sm text-secondary-foreground">Source: {backup.source}</p>
            <p className="text-sm text-secondary-foreground">Destination: {backup.destination}</p>
            <p className="text-sm text-secondary-foreground">Fréquence: {backup.frequency}</p>
            <p className="text-sm text-secondary-foreground">Statut: {backup.status}</p>
            <p className="text-sm text-secondary-foreground">Dernière sauvegarde: {backup.last_backup}</p>
